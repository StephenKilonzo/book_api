...
api=Api(app, doc='/', title='Books API', description="Books API")
...
class BookResource(Resource):
    ...
    @api.marshal_with(book_model, code=200, envelope="book")
    def put(self, id):
        book_to_update=Book.get_or_404(id)

        data=request.get_json()

        book_to_update.title=data.get('title')
        book_to_update.author=data.get('title')
        return book_to_update, 200


    @api.marshal_with(book_model, code=200, envelope="boo_deleted")
    def delete(self, id):
